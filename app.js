...
function fmtTime12h(ts) {
  ...
  return `${pad(d.getMonth()+1)}/${pad(d.getDate())} ${ampm} ${h}:${pad(d.getMinutes())}`;
}
...
const Chat = {
  ...
  render() {
    ...
    const showAvatar = settings.showChatAvatar;
    const fmt = settings.timestampStyle === 'sec' ? fmtTimeSec : settings.timestampStyle === '12h' ? fmtTime12h : fmtTime;
    container.innerHTML = msgs.map(m => {
      if (m.recalled) {
        const isUser = m.sender === 'user';
        return `<div class="system-message">${isUser ? '你' : settings.characterName} 撤回了一条消息</div>`;
      }
      const isUser = m.sender === 'user';
      ...
      if (m.quoteId) {
        ...
        if (quoted && !quoted.recalled) {
          ...
          quoteHtml = `<div class="msg-quote">${quoted.sender === 'user' ? '你' : settings.characterName}: ${qText}${quoted.content.length > 50 ? '…' : ''}</div>`;
        }
      }
      const displayContent = this._renderContent(m.content);
      const avatarHtml = showAvatar ? `<span class="msg-avatar">${isUser ? settings.userAvatar : settings.characterAvatar}</span>` : '';
      if (isUser) {
        return `<div class="message ${cls}${extraCls}" data-id="${m.id}">
          ...
          <div class="msg-content-wrap">
            ...
            <div class="msg-time-ext">${fmt(m.timestamp)}${star}</div>
          </div>
        </div>`;
      }
      return `<div class="message ${cls}${extraCls}" data-id="${m.id}">
...
const Settings = {
  ...
  load() {
    ...
    document.getElementById('settings-show-chat-avatar').checked = s.showChatAvatar !== false;
    document.getElementById('settings-timestamp-style').value = s.timestampStyle || 'default';
  ...
  _updateHeader() {
    ...
    this._setAvatar('header-user-avatar', s.userAvatar);
  ...
  save() {
    ...
    s.timestampStyle = document.getElementById('settings-timestamp-style').value;
  ...
  bindEvents() {
    ...
    document.getElementById('settings-show-chat-avatar').addEventListener('change', () => this.save());
    document.getElementById('settings-timestamp-style').addEventListener('change', () => this.save());
...
function init() {
  ...
  // User info modal
  document.getElementById('header-user-avatar').addEventListener('click', () => {
    const s = Store.getSettings();
    Cards._populateEmojiPicker(document.getElementById('user-info-emoji-picker'), true);
    document.getElementById('user-info-name').value = s.userName;
    Settings._setAvatar('user-info-avatar', s.userAvatar);
    showModal('user-info-modal');
  });
  document.getElementById('user-info-emoji-picker').addEventListener('click', (e) => {
    const item = e.target.closest('.emoji-item');
    if (!item) return;
    document.querySelectorAll('#user-info-emoji-picker .emoji-item').forEach(el => el.style.borderColor = 'var(--border)');
    item.style.borderColor = 'var(--primary)';
  });
  document.getElementById('user-info-save').addEventListener('click', () => {
    const s = Store.getSettings();
    const name = document.getElementById('user-info-name').value.trim();
    if (name) s.userName = name;
    const selected = document.querySelector('#user-info-emoji-picker .emoji-item[style*="var(--primary)"]');
    if (selected) {
      s.userAvatar = selected.dataset.text;
    }
    Store.saveSettings(s);
    Settings._updateHeader();
    hideModal('user-info-modal');
    toast('个人信息已更新');
  });
  ...
  // Icon changer
  const ICON_BUTTONS = [
    { key: 'call', label: '打电话', default: '📞' },
    { key: 'video', label: '视频通话', default: '📹' },
    { key: 'mood', label: '心情手账', default: '📖' },
    { key: 'favorites', label: '收藏', default: '⭐' },
    { key: 'menu', label: '菜单', default: '☰' },
    { key: 'nav-chat', label: '导航-对话', default: '💬' },
    { key: 'nav-cards', label: '导航-字卡', default: '🃏' },
    { key: 'nav-letters', label: '导航-信件', default: '✉️' },
    { key: 'nav-settings', label: '导航-设置', default: '⚙️' }
  ];

  function getIconMap() { return Store._get('iconMap') || {}; }
  function saveIconMap(map) { Store._set('iconMap', map); }

  function applyIconMap() {
    const map = getIconMap();
    ICON_BUTTONS.forEach(btn => {
      const val = map[btn.key] || btn.default;
      const el = document.querySelector(`[data-icon-key="${btn.key}"]`);
      if (!el) return;
      if (val.startsWith('data:image/') || val.startsWith('blob:')) {
        el.innerHTML = `<img src="${val}" style="width:100%;height:100%;object-fit:cover;" alt="">`;
  ...
  function openIconChanger() {
    const list = document.getElementById('icon-list');
    const map = getIconMap();
    list.innerHTML = ICON_BUTTONS.map(btn => {
      const current = map[btn.key] || btn.default;
      return `<div class="icon-row" data-key="${btn.key}" style="display:flex;align-items:center;gap:8px;padding:6px 4px;border-bottom:1px solid var(--border);cursor:pointer;">
        <span style="font-size:20px;width:30px;text-align:center;">${current}</span>
        <span style="flex:1;font-size:14px;">${btn.label}</span>
        <span style="font-size:12px;color:var(--text-secondary);">点击更换</span>
      </div>`;
    }).join('');
    document.getElementById('icon-picker-area').classList.add('hidden');

    list.querySelectorAll('.icon-row').forEach(row => {
      row.addEventListener('click', () => {
        const key = row.dataset.key;
        const pickerArea = document.getElementById('icon-picker-area');
        pickerArea.classList.remove('hidden');
        document.getElementById('icon-picker-label').textContent = `选择 "${ICON_BUTTONS.find(b => b.key === key).label}" 的新图标`;
        const grid = document.getElementById('icon-picker-grid');
        const builtinIcons = ['💬', '💌', '✨', '⭐', '🌙', '🌸', '🎀', '💎', '🌈', '🦋', '🍀', '🌺', '🎵', '📖', '🕊️', '🌟', '❤️', '💫', '🎶', '🍃', '🌊', '🔥', '⚡', '💧'];
        grid.innerHTML = builtinIcons.map(icon =>
          `<span class="emoji-item" data-icon="${icon}">${icon}</span>`
        ).join('');
        grid.querySelectorAll('.emoji-item').forEach(el => {
          el.addEventListener('click', () => {
            ...
            applyIconMap();
            openIconChanger();
            toast(`已更换 ${ICON_BUTTONS.find(b => b.key === key).label} 图标`);
          });
        });
        document.getElementById('icon-upload-btn').onclick = () => document.getElementById('icon-file-input').click();
    ...
    showModal('change-icon-modal');
  }

  // Apply icon map on load
  applyIconMap();
